...
public class StreamingAudioFileSpeechToText : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        soundPath = Application.streamingAssetsPath + "/Sound/Recording-13.wav";
    ...
    {
        byte[] audioBytes = File.ReadAllBytes(audioFilePathInputField.text);
        _service.StartListening(OnRecognize, OnRecognizeSpeaker);

        AudioData record = new AudioData();
        record.Clip = WaveFile.ParseWAV("myClip", audioBytes);
        float[] samples = new float[record.Clip.samples * record.Clip.channels];
        record.Clip.GetData(samples, 0);
        record.MaxLevel = Mathf.Max(Mathf.Abs(Mathf.Min(samples)), Mathf.Max(samples));
        Debug.Log("MaxLevel = " + record.MaxLevel);
        PlayClip(record.Clip);
    ...
    private void OnRecognizeSpeaker(SpeakerRecognitionEvent result)
    {
        if (result != null)
        {
            foreach (IBM.Watson.SpeechToText.V1.SpeakerLabelsResult labelResult in result.speaker_labels)
            {
